...
email_check_consumer.on('message', function(message){
  ...
  user_db.find({email: form_values.email}).toArray(function(err, results) {
    if (err) throw err;
...
signup_consumer.on('message', function(message){
  ...
  bcrypt.genSalt(saltRounds, function(err, salt) {
    bcrypt.hash(form_values.password, salt, function(err, hash) {
      user_db.insertOne({id: id, name: form_values.name, password: hash, email: form_values.email}, function(err, res) {
        if (err) throw err;
        user_db.findOne({email: form_values.email}, function(err, results){
          if (err) throw err;
          console.log(db);
          console.log(results);
